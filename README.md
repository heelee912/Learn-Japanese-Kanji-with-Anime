...
애니메이션을 오래 봐서 듣는 것에는 큰 무리가 없지만(오래 봐왔다면 해당 방법 자체의 적합도가 높기도) 한자를 잘 모르는 사람에게 효과가 가장 좋을 것이라고 생각합니다.
...
자막 파일과 같은 경로에 두고 실행 시 전체 싱크 및 특정 문구 부터의 싱크를 개별적으로 지정할 수 있습니다.(특정 문구 쪽도 상대값이 아닌 절대값 입력)
